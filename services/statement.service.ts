...
interface GenerateStatementParams {
    accountId: string;
...
const buildStatementHtml = (params: {
    accountHolderName: string;
    balance: number;
    ...
    return `
        ...
        <body>
            ...
            <div class="footer">
                Documento generado por Magnate · ${escapeHtml(new Date().getFullYear().toString())}
            </div>
        </body>
        </html>`;
};
...
export const statementService = {
     ...
     * y lo comparte para guardar/enviar.
     */
    async generateAndShare(params: GenerateStatementParams): Promise<StatementResult> {
        ...
        let totalExpense = 0;
        movements.forEach((m) => {
            if (m.movement_type === 'income') totalIncome += m.amount;
            else if (m.movement_type === 'expense') totalExpense += m.amount;
        });
